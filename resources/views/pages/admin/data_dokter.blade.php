...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5 flex flex-col sm:flex-row justify-between gap-3">
        <div class="relative w-full sm:w-1/3"> 
            ...
            <input type="text" id="searchDokter" placeholder="Cari email, nama, STR, atau HP..." class="w-full bg-gray-50 rounded-xl pl-10 pr-9 py-2.5 text-sm focus:ring-2 focus:ring-[#09637E]/10 focus:border-[#09637E]/40 outline-none border border-gray-200">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <button onclick="openTambah()" class="bg-[#09637E] text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-[#074d61] transition"> + Tambah Dokter </button>
    </div>
            ...
            <table class="w-full text-sm text-left min-w-[600px]">
                ...
                <tbody id="tableDokter" class="text-gray-700 divide-y divide-gray-100"> 
                    ...
                    <tr data-dokter-id="{{ $dokter->id }}" class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-5 py-3.5 font-medium nama">{{ $dokter->name }}</td>
                        <td class="px-5 py-3.5 str">{{ $dokter->no_str ?? '-' }}</td>
...
<div id="modalTambah" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-md flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1">
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Email</label>
                    <input type="email" id="tEmail" placeholder="dr.sarah@meditech.local" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#09637E]/10 focus:border-[#09637E]/40">
                </div>
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Nama Lengkap</label> 
                    <input type="text" id="tNama" placeholder="Dr. Sarah Wijaya" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#09637E]/10 focus:border-[#09637E]/40">
                </div>
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">No. STR</label> 
                ...
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">No. HP</label> 
                ...
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Status</label>
                ...
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Password</label>
...
<div id="modalEdit" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-md flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1">
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Email</label>
                    <input type="email" id="eEmail" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#09637E]/10 focus:border-[#09637E]/40">
                </div>
                <div> <label class="block text-xs text-gray-400 uppercase tracking-wide font-medium mb-1.5">Nama Lengkap</label> 
...
<script> 
    ...
    function showAlert(icon, title, text) { Swal.fire({ icon: icon, title: title, text: text, timer: 2000, showConfirmButton: false }); } 

    function tampilSemua() { let t = 0; document.querySelectorAll('#tableDokter tr').forEach(function (r) { r.style.display = ''; t++; }); document.getElementById('infoJumlah').textContent = 'Menampilkan ' + t + ' data dokter'; document.getElementById('btnClearSearch').classList.add('hidden'); } 
    function clearSearch() { document.getElementById('searchDokter').value = ''; tampilSemua(); } 
    function cariDokter() { let v = document.getElementById('searchDokter').value.toLowerCase().trim(); if (!v) { tampilSemua(); return; } let t = 0; document.querySelectorAll('#tableDokter tr').forEach(function (r) { let e = r.querySelector('.email') ? r.querySelector('.email').innerText.toLowerCase() : ''; let n = r.querySelector('.nama') ? r.querySelector('.nama').innerText.toLowerCase() : ''; let s = r.querySelector('.str') ? r.querySelector('.str').innerText.toLowerCase() : ''; let h = r.querySelector('.hp') ? r.querySelector('.hp').innerText.toLowerCase() : ''; let c = e.includes(v) || n.includes(v) || s.includes(v) || h.includes(v); r.style.display = c ? '' : 'none'; if (c) t++; }); document.getElementById('infoJumlah').textContent = 'Menampilkan ' + t + ' data dokter'; document.getElementById('btnClearSearch').classList.remove('hidden'); } 
    document.getElementById('searchDokter').addEventListener('input', function () { if (searchTimeout) clearTimeout(searchTimeout); if (!this.value.toLowerCase().trim()) { tampilSemua(); } else { searchTimeout = setTimeout(cariDokter, 400); } }); 

    function openTambah() { ['tEmail', 'tNama', 'tSTR', 'tHP', 'tPass'].forEach(id => document.getElementById(id).value = ''); document.getElementById('tStatus').value = 'Aktif'; document.getElementById('modalTambah').classList.remove('hidden'); document.getElementById('modalTambah').classList.add('flex'); document.body.style.overflow = 'hidden'; } 
    function closeTambah() { document.getElementById('modalTambah').classList.add('hidden'); document.getElementById('modalTambah').classList.remove('flex'); document.body.style.overflow = ''; } 
    ...
    async function simpanDokter() { 
        ...
        if (!email || !nama || !password) { 
            showAlert('error', 'Gagal', 'Email, Nama, dan Password wajib diisi!'); 
            return; 
        } 
        ...
        const response = await fetch(storeUrl, { 
            ...
            body: JSON.stringify({ email, name: nama, no_str: str, no_hp: hp, password, status: status }), 
        }); 

        if (!response.ok) { 
            const result = await response.json().catch(() => ({})); 
            // Kalau validasi Laravel gagal, biasanya kirim errors object
        ...
        document.getElementById('tableDokter').insertAdjacentHTML('beforeend', '<tr data-dokter-id="' + data.id + '" class="hover:bg-gray-50 transition">' + '<td class="px-5 py-3.5 text-xs text-gray-500 email">' + data.email + '</td>' + '<td class="px-5 py-3.5 font-medium nama">' + data.name + '</td>' + '<td class="px-5 py-3.5 str">' + (data.no_str || '-') + '</td>' + '<td class="px-5 py-3.5 hp">' + (data.no_hp || '-') + '</td>' + '<td class="px-5 py-3.5"><span class="px-3 py-1 text-xs rounded-full ' + badgeClass + ' font-medium">' + data.status + '</span></td>' + '<td class="px-5 py-3.5 text-right"><button onclick="openEdit(this)" class="px-3 py-1.5 rounded-xl text-xs font-semibold bg-[#09637E]/10 text-[#09637E] hover:bg-[#09637E] hover:text-white transition">Edit</button></td>' + '</tr>'); 
        
        closeTambah(); 
        showAlert('success', 'Berhasil', 'Dokter baru berhasil ditambahkan.'); 
    } 

    function openEdit(btn) { selRow = btn.closest('tr'); selRowId = selRow.dataset.dokterId; document.getElementById('eEmail').value = selRow.querySelector('.email').innerText; document.getElementById('eNama').value = selRow.querySelector('.nama').innerText; document.getElementById('eSTR').value = selRow.querySelector('.str').innerText; document.getElementById('eHP').value = selRow.querySelector('.hp').innerText; document.getElementById('eStatus').value = selRow.querySelector('span').innerText.trim(); document.getElementById('ePass').value = ''; document.getElementById('modalEdit').classList.remove('hidden'); document.getElementById('modalEdit').classList.add('flex'); document.body.style.overflow = 'hidden'; } 
    function closeEdit() { document.getElementById('modalEdit').classList.add('hidden'); document.getElementById('modalEdit').classList.remove('flex'); document.body.style.overflow = ''; } 
    ...
    async function updateDokter() { 
        ...
        if (!email || !nama) { 
            showAlert('error', 'Gagal', 'Email dan Nama wajib diisi!'); 
            return; 
        } 
        if (!selRowId) { showAlert('error', 'Error', 'Dokter tidak ditemukan.'); return; } 
        
        const url = updateUrlBase.replace('/0', '/' + selRowId); 
        const response = await fetch(url, { 
            method: 'PUT', 
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json', }, 
            body: JSON.stringify({ email, name: nama, no_str: str, no_hp: hp, status, password }), 
        }); 

        if (!response.ok) { 
            const result = await response.json().catch(() => ({})); 
            let errorMsg = result.message || 'Gagal memperbarui data.';
            if (result.errors) {
                errorMsg = Object.values(result.errors).flat().join('<br>');
            }
            showAlert('error', 'Gagal', errorMsg); 
        ...
        selRow.querySelector('.nama').innerText = data.name; 
        selRow.querySelector('.str').innerText = data.no_str || '-'; 
        selRow.querySelector('.hp').innerText = data.no_hp || '-'; 
        
        let badgeClass = data.status === 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'; 
        let statusSpan = selRow.querySelector('span'); 
        statusSpan.className = 'px-3 py-1 text-xs rounded-full ' + badgeClass + ' font-medium'; 
        statusSpan.innerText = data.status; 
        
        closeEdit(); 
        showAlert('success', 'Berhasil', 'Perubahan data dokter berhasil disimpan.'); 
    } 

    document.addEventListener('keydown', function (e) { if (e.key === 'Escape') { closeTambah(); closeEdit(); } }); 
</script>
@endsection