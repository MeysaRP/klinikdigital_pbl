...
function tampilSemua() {
    let t = 0;
    document.querySelectorAll('#tableDokter tr').forEach(function (r) { r.style.display = ''; t++; });
    document.getElementById('infoJumlah').textContent = 'Menampilkan ' + t + ' data dokter';
...
function cariDokter() {
    let v = document.getElementById('searchDokter').value.toLowerCase().trim();
    if (!v) { tampilSemua(); return; }
    let t = 0;
    document.querySelectorAll('#tableDokter tr').forEach(function (r) {
        let e = r.querySelector('.email') ? r.querySelector('.email').innerText.toLowerCase() : '';
        ...
        let c = e.includes(v) || n.includes(v) || s.includes(v) || h.includes(v);
...
document.getElementById('searchDokter').addEventListener('input', function () {
    ...
    if (!this.value.toLowerCase().trim()) { tampilSemua(); }
    else { searchTimeout = setTimeout(cariDokter, 400); }
});
...
function openEdit(btn) {
    ...
    document.getElementById('eEmail').value = selRow.querySelector('.email').innerText;
    document.getElementById('eNama').value = selRow.querySelector('.nama').innerText;
    document.getElementById('eSTR').value = selRow.querySelector('.str').innerText;
    document.getElementById('eHP').value = selRow.querySelector('.hp').innerText;
    document.getElementById('eStatus').value = selRow.querySelector('span').innerText.trim();
...
async function updateDokter() {
    ...
    if (!email || !nama || !str || !hp) {
        alert("Email, Nama, STR, dan HP harus diisi!");
        return;
    }
